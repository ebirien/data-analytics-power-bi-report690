...
4. Creation of the Data Model:
  - Created four Report Pages named as follows:
     - Executive Summary
     - Customer Detail
     - Product Detail
     - Stores Map
...
5. Building the Customer Detail Page
  - Created Headline Card Visuals
     ...
     - Created a new measure in the Measures Table called [Revenue per Customer] = DIVIDE([Total Revenue],[Total Customers])
  ...
  - Created the Top 20 Customers Table
     - Created a new table, which displays the top 20 customers, filtered by revenue. The table should show each customer's full name, revenue, and number of orders. See table creation formular below:
     - Top 20 Customers = TOPN(21,  SUMMARIZECOLUMNS (Customers[Full Name],
     "Total Revenue", SUMX(Orders, Orders[Product Quantity] * RELATED(Products[Sale Price])),
     "Total Orders", (COUNTROWS('Orders'))
     ), [Total Revenue], DESC)
